...
ENTRY_POINT_COLORS = {
    "Glycolysis":          "#BBD5E9",   # light blue
    "Pyruvate":            "#5B8C8F",   # teal
    "Acetyl-CoA":          "#EBB309",   # gold
    "TCA — α-KG":          "#E38D6B",   # salmon
    "TCA — OAA":           "#803E25",   # dark brick
    "TCA — Succinyl-CoA":  "#7E9B6B",   # muted olive (added)
...
def plot_growth_cue(summary_df: pd.DataFrame, out_path: Path) -> None:
    ...
    ax_g.set_title("MIT1002 growth rate and carbon-use efficiency across substrates",
                   fontsize=12, pad=8)
    ax_g.margins(x=0.01)
    ...
    handles = [mpatches.Patch(facecolor=ENTRY_POINT_COLORS[lbl], edgecolor="white",
                              label=lbl)
               for lbl in ENTRY_POINT_ORDER if lbl in df["entry_point"].values]
    ax_g.legend(handles=handles, title="Entry point into\ncentral metabolism",
...
def plot_growth_cue_correlation(summary_df: pd.DataFrame, out_path: Path) -> None:
    ...
    for _, row in df.iterrows():
        ...
        ax.annotate(row["substrate"], (row["growth_rate"], row["cue"]),
                    xytext=(dx, dy), textcoords="offset points",
                    fontsize=7, ha=ha, va=va, color="#555555")

    ax.set_xlabel("Growth rate (h⁻¹)")
    ax.set_ylabel("Carbon-use efficiency")
    ax.set_title("Growth rate vs. carbon-use efficiency across substrates",
                 fontsize=12, pad=8)

    # Correlation metrics
    txt = (f"Pearson  r = {r:.2f}  (p = {p:.2g})\n"
           f"Spearman ρ = {rho:.2f}  (p = {p_s:.2g})")
    ax.text(0.025, 0.025, txt, transform=ax.transAxes, fontsize=9,
    ...
    handles = [mpatches.Patch(facecolor=ENTRY_POINT_COLORS[lbl], edgecolor="white",
                              label=lbl)
               for lbl in ENTRY_POINT_ORDER if lbl in df["entry_point"].values]
    handles.append(Line2D([0], [0], color="#555555", ls="--", lw=1.2,
                          label="linear fit"))
    ax.legend(handles=handles, title="Entry point into\ncentral metabolism",
              frameon=False, bbox_to_anchor=(1.01, 1), loc="upper left",
              fontsize=9, title_fontsize=9.5)
